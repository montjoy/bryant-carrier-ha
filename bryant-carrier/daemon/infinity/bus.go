...
func (b *Bus) openSerial() error {
	log.Infof("opening serial interface: %s", b.device)
	if b.port != nil {
		b.port.Close()
		b.port = nil
	}
...
func (b *Bus) handleFrame(frame Frame) *Frame {
	// Every frame on the bus lands here several times a second.  logrus Printf
	// is Info level, which made the default log unusable; frame-level detail is
	// a debugging concern.
	log.Debugf("read frame: %s", frame)
...
func (b *Bus) reader() {
	...
	for {
		...
		if n == 0 || err != nil {
			// A zero-length read with no error is a read timeout, so err can
			// legitimately be nil here -- calling err.Error() unconditionally
			// would panic the reader goroutine on an idle bus.
			if err != nil {
				log.Warnf("error reading from serial port: %s", err.Error())
			} else {
				log.Debugf("serial read timed out, reopening %s", b.device)
			}
			if b.port != nil {
				b.port.Close()
			}
...
func (b *Bus) performAction(action *Action) {
	log.Debugf("encoded frame: %s", action.requestFrame)
	...
	for tries := 0; tries < responseRetries; {
		...
		case res := <-b.responseCh:
			...
			if action.requestFrame.op == ReadTableBlock && !bytes.Equal(reqTable, resTable) {
				log.Debugf("got response for incorrect table, is: %x expected: %x", resTable, reqTable)
				continue
			}
...
func (b *Bus) send(dst uint16, op uint8, requestData []byte, response interface{}) bool {
	...
	if ok && op == ReadTableBlock && act.responseFrame != nil && act.responseFrame.data != nil && len(act.responseFrame.data) > 6 {
		...
		if ok {
			log.Debugf("handling a RawRequest")
			*raw.Data = append(*raw.Data, act.responseFrame.data[6:]...)
			log.Debugf("raw data length is: %d", len(*raw.Data))
		} else {
