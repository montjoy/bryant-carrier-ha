...
func (params *TStatVacationParams) FromAPI(config *APIVacationConfig) byte {
	...
	if config.FanMode != nil {
		// An unrecognized fan mode used to fall through as 0 ("auto") and get
		// written anyway.  Leave the flag clear instead so the field is simply
		// not part of the write.
		if mode, ok := StringFanModeToRaw(*config.FanMode); ok {
			params.FanMode = mode
			flags |= 0x40
		}
	}
